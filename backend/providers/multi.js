...
export async function getModels(make) {
  ...
  const rows = await db(`SELECT DISTINCT model FROM v_catalogue WHERE make=$1 ORDER BY model`, [make]);
...
export async function getEngines(make, model, year) {
  try {
    ...
    console.warn(`CarQuery.getTrims(${make}, ${model}, ${year}) vide -> fallback DB`);
  } catch (e) {
    console.warn(`CarQuery.getTrims(${make}, ${model}, ${year}) ERROR:`, e.message);
  }
  const rows = await db(
    `SELECT id as trim_id, engine 
     FROM v_trim_quote 
